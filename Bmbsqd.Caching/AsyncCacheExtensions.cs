...
{
	private static async Task<KeyValuePair<TKey, TValue>> CreatePendingTask<TKey, TValue>(IAsyncCache<TKey, TValue> cache, List<KeyValuePair<TKey, TaskCompletionSource<TValue>>> pending, TKey key) {
	...
	public static async Task<IDictionary<TKey, TValue>> GetOrAddAsync<TKey, TValue>(
		this IAsyncCache<TKey, TValue> cache,
		IEnumerable<TKey> keys,
		Func<IEnumerable<TKey>, Task<IDictionary<TKey, TValue>>> factory
	) {
		var result = new List<Task<KeyValuePair<TKey, TValue>>>();
		var pending = new List<KeyValuePair<TKey, TaskCompletionSource<TValue>>>();

		foreach( var x in keys ) {
			result.Add(CreatePendingTask(cache, pending, x));
		}
		...
		try {
			if( pending.Any() ) {
				...
				foreach( var p in pending ) {
					if( remaining.TryGetValue(p.Key, out var value) ) {

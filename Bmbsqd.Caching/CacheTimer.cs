...
internal static class CacheTimer
{
	...
	private static void InvalidateExpiredCacheItems() {
		var list = GetSnapshot();
		foreach( var r in list ) {
			if( r.TryGetTarget(out var cache) ) {
	...
	private static void InitializeTimer() {
		lock(typeof(CacheTimer)) {
			_timer ??= new Timer(static _ => InvalidateExpiredCacheItems(), null, _period, _period);
		}
	}
}